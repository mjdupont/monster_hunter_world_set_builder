...
module UserArmorList = 
  ...
  [<ReactComponent>]
  let Component (props: {| Armor : (Armor * bool) list; SetArmor: ((Armor * bool) list -> unit) |}) = 
    let searchWord, setSearchWord = React.useState ""
    
    let updateArmor armorToChange newStatus = 
      props.Armor
      |> List.map (fun (armor, status) -> if armor = armorToChange then armor, newStatus else armor, status)
      |> props.SetArmor
    ...
    Html.div [ 
      ...
      prop.children ( 
        ...
        @ [ for (armor, doesUserHaveArmor) in (props.Armor |> List.filter (fun (armor, _) -> matchesByNameOrSkills searchWord armor)) -> 
            Html.div [ 
              prop.className "user-armor-selector flex justify-between"
              prop.children [ 
                Html.label 
                  [ prop.className "user-armor-selector-label truncate" 
                    prop.text armor.Name  
                  ]
                Html.input [
                  prop.id armor.Name
                  prop.type' "checkbox"
