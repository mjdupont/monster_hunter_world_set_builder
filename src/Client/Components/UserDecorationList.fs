...
module UserDecorationList = 
  ...
  [<ReactComponent>]
  let Component (props: {| Decorations : ((Decoration * int) * int) list; SetDecorations: ((Decoration * int) list -> unit) |}) =  
    let searchWord, setSearchWord = React.useState ""

    let updateCount decoration newCount = 
      props.Decorations 
      |> List.map (fun ((pDecoration, pCount), pMax) -> 
        if pDecoration = decoration 
        then pDecoration, newCount
        else pDecoration, pCount
        )
    ...
    Html.div [ 
      ...
      prop.children ( 
        ...
        @ [ for ((decoration, count), max) in (props.Decorations |> List.filter (fun ((decoration, count), max) -> matchesByNameOrSkills searchWord decoration)) -> 
            Html.div [ 
