...
module Sidebar =
    ...
    let Component openName (props: {| Content: ReactElement |}) =
        ...
        let toggleSidebar _mouseEvent =
            if isOpen then setIsOpen false else setIsOpen true
        ...
        Html.div [
            prop.className
                $"sidebar {status} fixed top-0 left-0 h-full bg-gray-800 text-white transition-transform duration-300 {transition} w-64 z-50"
            prop.children [
                Html.div [
                    prop.children [
                        Html.button [
                            let transition_transform_button =
                                if isOpen then "translate-x-0" else "translate-x-full"
