...
module Accordion =
  ...
  let private AccordionTab ( props: {| Title: string; SetTab: (unit -> unit) ; IsActive: bool |}) = 
      ...
      Html.div [
        ...
        prop.children [
          Html.div [
            ...
            let tabsAsElements = tabs |> List.mapi (fun i tab -> AccordionTab {| Title = tab.Title; SetTab = (fun () -> setActiveTab i); IsActive = i = activeTab |}) 
            prop.children tabsAsElements
          ]
          tabIndices |> Map.tryFind activeTab |> Option.map (fun ac -> contentWrapper ac.Content ) |> Option.defaultValue (Html.text "Tab Index out of bounds! This should not happen.")
        ]
      ]