...
module SearchStatus =
  ...
  let Component 
    ...
    if props.Status = Failed || props.Status = Found 
    then Fable.Core.JS.setTimeout (fun () -> props.SetSearchStatus NotAsked) 10000 |> ignore
    //else 0 |> ignore
    ...
    Html.div [
      prop.children [
        Html.h3 
          [ prop.style [ style.color statusColor]
            prop.text message
          ]
      ]
    ]