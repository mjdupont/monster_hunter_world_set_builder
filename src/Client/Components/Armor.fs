                    ...
                    prop.children [
                        SelectSearch.selectSearch [
                            selectSearch.value (
                                ...
                                |> Option.map (fun (armor, decorations) -> armor.Id |> sprintf "%i")
                                |> Option.defaultValue ""
                            )
                            selectSearch.placeholder "Select an Armor Piece"
                            selectSearch.search true
                            selectSearch.onChange (
                                findPieceFromId >> updateArmorIfDifferent >> props.ChosenArmor.Update
                            )
                        ...
                        Html.div [
                            Html.text (
                                ...
                                + (sprintf
                                    "F: %s "
                                    (props.ChosenArmor.Value
                                     |> Option.map (fun (armor, decorations) -> armor.Resistances.Fire |> string)
                                     |> Option.defaultValue "-"))
                                + (sprintf
                                    "W: %s "
                                    (props.ChosenArmor.Value
                                     |> Option.map (fun (armor, decorations) -> armor.Resistances.Water |> string)
                                     |> Option.defaultValue "-"))
                                + (sprintf
                                    "T: %s "
                                    (props.ChosenArmor.Value
                                     |> Option.map (fun (armor, decorations) -> armor.Resistances.Thunder |> string)
                                     |> Option.defaultValue "-"))
                                + (sprintf
                                    "I: %s "
                                    (props.ChosenArmor.Value
                                     |> Option.map (fun (armor, decorations) -> armor.Resistances.Ice |> string)
                                     |> Option.defaultValue "-"))
                                + (sprintf
                                    "D: %s "
                                    (props.ChosenArmor.Value
                                     |> Option.map (fun (armor, decorations) -> armor.Resistances.Dragon |> string)
                                     |> Option.defaultValue "-"))
                            )
                        ]
                    ]
