...
module UserCharmList = 
  ...
  [<ReactComponent>]
  let Component (props: {| Charms : (Charm * int) list; SetCharms: ((Charm * int) list -> unit) |}) = 
    let searchWord, setSearchWord = React.useState ""
    
    let updateCharms charmToChange newMaxRank = 
      props.Charms
      |> List.map (fun (charm, maxRank) -> if charm = charmToChange then charm, newMaxRank else charm, maxRank)
      |> props.SetCharms
      ...
      prop.children ( 
          ...
          |> List.filter (fun (charm, maxRank) -> 
            matchesByNameOrSkillsByPredicate 
              ...
              (fun charm -> 
                charm.Ranks 
                |> Array.map 
                  (fun cr -> cr.Skills |> Array.map (fun sr -> sr.SkillName)) 
                  |> Array.concat 
                  |> List.ofArray)
              searchWord 
              charm
        ...
        @ [ for (charm, maxOwnedRank) in filteredCharms -> 
            Html.div [ 
              ...
              prop.children [ 
                Html.label 
                  [ prop.className "user-charm-selector-label truncate" 
                    prop.text charm.Name  
                  ]
                Html.input [
                  prop.id charm.Name
                  prop.type' "number"
