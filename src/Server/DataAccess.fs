...
module InferredTypes =
    ...
    module SkillRank =
        ...
        [<Literal>]
        let sample =
            """{"id":39,"slug":"attack-boost-rank-1","skill":15,"level":1,"description":"Attack +3","modifiers":{"attack":3}}"""
    ...
    module Skill =
        ...
        [<Literal>]
        let sample =
            """{"id":15,"slug":"attack-boost","name":"Attack Boost","description":"Increases attack power. Also ...","ranks":[{"id":39,"slug":"attack-boost-rank-1","skill":15,"level":1,"description":"Attack +3","modifiers":{"attack":3}}]}"""
        ...
        let loadSkills: Async<(Map<int, string> * Skill list)> = async {
            printfn "Loading Skills..."
            let! skills = JsonValue.AsyncLoad(uri)
            let skills = skills |> (fun x -> x.ToString()) |> Api.ParseList |> Array.ofSeq

            let skillDict = buildSkillDict skills
            let realSkills = skills |> Array.map (toReal skillDict) |> List.ofSeq
    ...
    module Decoration =
        ...
        [<Literal>]
        let sample =
            """{"id":1,"rarity":5,"slot":1,"name":"Antidote Jewel 1","skills":[{"id":1,"level":1,"modifiers":{},"description":"Reduces the number of times you take poison damage.","skill":1,"skillName":"Poison Resistance"}]}"""
        ...
        let toReal skillNames (deco: Api.Root) : Decoration =
            let string =
                deco.Skills |> Array.map (fun skill -> skill.JsonValue |> (_.ToString()))
        ...
        let getIconsFromHtml: Async<(string * string) list> = async {
                ...
                Regex(
                    System.IO.File.ReadAllLines(
                        System.IO.Path.Join [|
                            ...
                            Resources.decorationImageRegexPath
                        |]
                    )
                    |> String.concat ""
                )
            ...
            let htmlLines =
                System.IO.File.ReadAllLines(
                    System.IO.Path.Join [| System.Environment.CurrentDirectory; Resources.decorationHtm |]
                )

            let htmlLines = Regex.Split(htmlLines |> String.concat "\n", "<td>")

            let matches =
                [ for htmlLine in htmlLines -> htmlRegex.Match(htmlLine) ]
        ...
        let obsoleteJewels = [ "Smoke Jewel 1" ]

        let private findMatchedUris decorations = async {
            ...
            let matchDecorations (matchedDecorations, unmatchedDecorations) (decoration: Decoration) =
                match
                    captureUris
                    |> List.filter (fun (name, url) -> name.ToLowerInvariant() = decoration.Name.ToLowerInvariant())
                with
                | [ (matchingName, matchingUrl) ] ->
                    ((matchingName, decoration, matchingUrl) :: matchedDecorations, unmatchedDecorations)
        ...
        let bindIcons (decorations: Decoration seq) : Async<Decoration list> =
            ...
            async {
                ...
                return
                    matchingDecorations
                    |> List.map (fun (_name, decoration, uri) -> {
                        decoration with
                            IconUri = Some(System.IO.Path.Join([| rootPath; uri |]))
                    })
                    |> List.ofSeq
    ...
    module Charm =
        ...
        [<Literal>]
        let sample =
            """{"id":234,"slug":"poison-charm","name":"Poison Charm","ranks":[{"level":1,"rarity":3,"skills":[{"id":1,"slug":"poison-resistance-rank-1","level":1,"description":"Reduces the duration of poison by 30%.","skill":1,"skillName":"Poison Resistance","modifiers":[]}],"crafting":{"craftable":true,"materials":[{"quantity":1,"item":{"id":231,"name":"Pukei-Pukei Sac","description":"Pukei-Pukei material. Obtained by ...","rarity":0,"carryLimit":0,"sellPrice":0,"buyPrice":0}}]}}]}"""
        ...
        let loadCharms (skillNames: Map<int, string>) : Async<Charm list> = async {
    ...
    module ArmorSet =
        ...
        let loadArmorSets (skillNames: Map<int, string>) : Async<ArmorSet list> = async {
...
module Testing =
    ...
    let explore_armor (armor: InferredTypes.Armor.Api.Root array) =
        ...
        let armorBySkillPotential =
            armorMap
            |> Map.map (fun armorSlot armor ->
                armor
                |> Array.ofSeq
                |> Array.sortByDescending InferredTypes.Armor.naiveSkillPotential)
    ...
    let explore_armorset_skills
        (armorsets: InferredTypes.ArmorSet.Api.Root array)
        (skills: InferredTypes.Skill.Api.Root array)
        =
        ...
        let armorSetSkills =
            armorsetsWithSkills
            |> Seq.map (fun armorSet -> armorSet.Ranks |> Seq.map (fun rank -> rank.Skill))
            |> Seq.concat

        let nonArmorSetSkills =
            let armorSetSkillIds =
                armorSetSkills |> Seq.map (fun armorSetSkill -> armorSetSkill.Skill)
    ...
    let explore_decorations (decorations: InferredTypes.Decoration.Api.Root array) =
        ...
        for slot, decogroup in groupedDecos do
            ...
            for deco in decogroup do
                printfn "%s" deco.Name


        let doublyGroupedDecos =
            groupedDecos
            |> Seq.map (fun (size, decos) -> (size, decos |> Seq.groupBy (fun deco -> deco.Rarity) |> Map.ofSeq))
            |> Map.ofSeq
        ...
        let fourSlotDecos =
            groupedDecos |> Seq.filter (fun (slot, decos) -> slot = 4) |> Seq.head |> snd
        ...
        printfn
            ...
            (skillsInMultiSkillDecos |> Seq.length)
            (multipleSkillsInFourSlotDecos |> Seq.length)

        for skill in skillsInMultiSkillDecos do
            let decosContainingThisSkill =
                multipleSkillsInFourSlotDecos
                |> Seq.filter (fun skills ->
                    skills |> Seq.filter (fun skill' -> skill'.Id = skill.Id) |> Seq.length > 0) in

            printfn "%s, (%i decorations)" skill.SkillName (decosContainingThisSkill |> Seq.length)
        ...
        let sharedNeighborsPairs =
            sharedNeighbors
            |> Map.map (fun neighbors skillPairs -> skillPairs |> Seq.map fst |> Array.ofSeq)
