...
type NumberedDecoration = { Id: int; Decoration: Decoration }

type NumberedSlot = { Id: int; Slot: Slot }
...
module Adjacency =
    ...
    let isSame adj1 adj2 = edge adj1 = edge adj2
...
let assembleMaxFlowByMap
    ...
    let decorationNodes =
        decorationCapacity |> List.map fst |> List.map snd |> Set.ofList
                    ...
                    for decorationNode in decorationNodes do
                        ...
                        if decoration.Slot <= slotSize then
                            ...
                            let capacity =
                                (edge,
                                 List.min [
                                     (if slotSize = 4 then 2 else 1)
                                     (decoration.Skills |> Array.sumBy (fun sr -> sr.Level))
                                 ])
            ...
            for slotNode in slotNodes do
                ...
                let capacity =
                    (edge,
                     slotNode
                     |> function
                         | DecorationSlot s when s.Slot = Slot 4 -> 2
                         | DecorationSlot s -> 1
                         | _ -> 0)
    ...
    let adjacencies =
        [ skills; decorations; slots; sink ]
        |> List.concat
        |> List.groupBy (fun (node, adjacency) -> node)
        |> List.map (fun (node, originalAdjacencies) -> node, originalAdjacencies |> List.map snd)

    let capacities =
        [ skillCapacity; decorationCapacity; slotCapacity; sinkCapacity ] |> List.concat

    let flows = [ skillFlow; decorationFlow; slotFlow; sinkFlow ] |> List.concat

    {
        Adjacency = adjacencies |> Map.ofList
        Capacity = capacities |> Map.ofList
        Flow = flows |> Map.ofList
        DecorationAssignmentLimit = slotNodes |> Seq.length
    }
...
let decorationAssignmentEdges (graph: MaxFlowByMap) =
    ...
    |> List.filter (function
        | (Decoration d, DecorationSlot ds) -> true
        | _ -> false)
...
let addPathFlow (path: Path) pathFlow (graph: MaxFlowByMap) =
    let updateGraph graph adjacency =
        ...
        | Forward(src, sink) -> {
            graph with
                Flow = graph |> addForwardFlow pathFlow (src, sink)
          }
        | Reverse(src, sink) -> {
            graph with
                Flow = graph |> addReverseFlow pathFlow (src, sink)
          }
...
let nAdjacenciesInPath (path: Path) (adj: Adjacency) =
    path
    |> List.filter (fun pAdj -> Adjacency.edge pAdj = Adjacency.edge adj)
    |> List.length
...
let oppositeDirectionInPath currentPath adjacency =
    currentPath
    |> List.exists (fun pathAdjacency ->
        not (pathAdjacency = adjacency)
        && (Adjacency.edge pathAdjacency = Adjacency.edge adjacency))
...
let rec findPathDFS (path: Path) (graph: MaxFlowByMap) : Path option =
    ...
    let findNextAdjacency (graph: MaxFlowByMap) (currentPath: Path) (node: NodeType) =
        ...
        let possibleNextMoves =
            nextNodes
            |> List.filter (fun adj ->
                fewerOccurrancesThanCapacities graph currentPath adj
                && not (oppositeDirectionInPath currentPath adj))
    ...
    | Forward(src, sink) :: rest when sink = Sink && (graph |> assignFlowFromPath path) |> withinDecorationSlotLimit ->
        Some path
    | adjacency :: rest ->
...
let rec findPathBFS (paths: Path list) (graph: MaxFlowByMap) : Path option =
    let findNextAdjacencies (path: Path) : Path list =
        ...
        let possibleNextMoves =
            nextNodes
            |> List.filter (fun adj ->
                fewerOccurrancesThanCapacities graph path adj
                && not (oppositeDirectionInPath path adj))
    ...
    printfn
        "BFS depth %i: %i paths found"
        (nextPaths |> List.tryHead |> Option.defaultValue [] |> List.length)
        (nextPaths |> List.length)

    let completingNextPaths =
        nextPaths
        |> List.filter (fun path ->
            match path with
            | Forward(src, sink) :: rest when
                sink = Sink && graph |> assignFlowFromPath path |> withinDecorationSlotLimit
                ->
                true
            | _ -> false)
...
let printPath (path: Path) =
    ...
    orderedPath
    |> List.fold (fun (outputString: string) adj -> outputString + (printAdjacency adj)) "Source"
...
let decorationSkillLevel skill decoration =
    ...
    | SingleDecoration(dSkill, level) -> if dSkill = skill then level else 0
...
let assign_decoration (state: DecorationAssignmentState) : AssignmentResult =
    let emptySlots =
        state.DecorationSlots |> List.filter (fun (slot, deco) -> deco.IsNone)
    ...
    let skillBalance = [
        for skill, requestedAmount in state.RequestedSkills do
            let assignedPoints =
                assignedSkills
                |> List.tryFind (fun (sk, i) -> sk = skill)
                |> Option.map snd
                |> Option.defaultValue 0
    ...
    | [] ->
        Done(
            state.DecorationSlots
            |> List.choose (fun (slot, deco) -> deco |> Option.map (fun dec -> slot, dec))
        )
    | slots ->
        let nextSkillToAssign =
            skillBalance |> List.tryFind (fun (skill, balance) -> balance > 0)
        ...
        | Some(skill, remainingSkillPointsNeeded) ->
            let decorationsWithThisSkill =
                state.Decorations |> Map.tryFind skill |> Option.defaultValue []

            let decorations =
                decorationsWithThisSkill
                |> List.groupBy (fun (decoration, count) -> decorationSkillLevel skill decoration)
...
let maximumPossibleSkillAssignment
    ...
    let fullyMatchedDecoration skills decoration =
        ...
        | PairDecoration((sk1, lv1), (sk2, lv2)) ->
            [ sk1; sk2 ] |> List.forall (fun decoSkill -> skills |> List.contains decoSkill)
