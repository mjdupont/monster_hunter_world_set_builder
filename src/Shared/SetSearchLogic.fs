...
let skillPointsFromSlot = (function | Slot 4 -> 2 | _ -> 1)
...
let anonymizeArmor armor = 
  { armor with ArmorSet = -1; Id = -1; Skills = [||]; Name = "Any Piece With These Slots"; Slug = "Placeholder" }
...
let armorSkillContribution (selectedSkillIds: int list) (armor:Armor) : int =
  let matchingArmorSkills = armor.Skills |> Array.filter (fun armorSkillRank -> selectedSkillIds |> List.contains armorSkillRank.Id )
  matchingArmorSkills |> Array.map (fun armorSkillRank -> armorSkillRank.Level) |> Array.sum
...
let anyPiece (armorType:ArmorType) : Armor = 
  { ArmorSet = -1
    ...
    Rank = Low
    Rarity = -1
    Resistances = { Fire = 0; Ice = 0; Water = 0; Dragon = 0; Thunder = 0 }
    Skills = [||]
    Slug = "Any Piece"
    Slots = [||]
    Type = armorType
