...
let armorSkillContribution skills decorations remainingSkillNeed (armor: Armor) =
    ...
    let decoContribution =
        armor.Slots |> asCounts |> calculateReach skills decorations remainingSkillNeed
...
let assignArmor
    ...
    let validArmor =
        armor |> List.filter (fun a -> unassignedPieces |> List.contains a.Type)
    ...
    | (bestArmor, armorContrib) :: restArmor, [] ->
        Some(
            ChosenSet.setArmor bestArmor.Type (Some(bestArmor, bestArmor.Slots |> DecorationSlots.FromSlots)) chosenSet
        )
    ...
    | [], (bestCharm, charmContrib) :: restCharm ->
        Some(
            {
                chosenSet with
                    Charm = Some(bestCharm, bestCharm.Ranks |> Array.maxBy (fun (cr: CharmRank) -> cr.Level))
            }
        )
...
let rec findSet
    ...
    | Some assignment ->
        ...
        | None ->
            ...
            | Some newChosenSet ->
                findSet skills decorations (weapon, weaponSlots) requestedSkills newChosenSet armor charms
            | None -> None
