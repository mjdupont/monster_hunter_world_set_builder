...
open DataTypes
  ...
  let assembleGraph (wantedSkills: (Skill * int) list) (decorationMap : Map<Skill, Map<Decoration, int>>) (decorationSlots : Slot list) : MaxFlowPlot = 
    ...
    let skills, skillQuantityEdges = 
      [ for skill, requestedCount in wantedSkills ->
        Skill skill, {Source = Source; Sink = Skill skill; Capacity = requestedCount}
      ]
      |> List.unzip
    ...
    let decorations, skillToDecorationAssignment = 
      [ for KeyValue (skill, decorationCounts) in decorationMap do 
          for KeyValue (decoration, count) in decorationCounts do
            [ for i in 0..count ->
                Decoration decoration, { Source = Skill skill; Sink = Decoration decoration; Capacity = skillToDecorationCapacity skill decoration}
            ]
      ] 
    ...
    let slots, decorationToSlotAssignment = 
      [ for ((DataTypes.Slot slotSize) as slot) in decorationSlots do 
          Slot slot, [ for decoration in decorations -> { Source = decoration; Sink = Slot slot; Capacity = (if slotSize = 4 then 2 else 1)}]
      ] 
      |> List.unzip
    ...
    let slotToSinkEdges = 
      [ for node in slots do
        match node with 
        | Slot ((DataTypes.Slot slotSize) as slot) -> yield {Source = Slot slot; Sink = Sink; Capacity = (if slotSize = 4 then 2 else 1)}
        | _ -> ()
      ]

    let sink = [Sink]

    let nodes = [source; skills; decorations; slots; sink] |> List.concat |> List.mapi (fun i node -> {Id = i; Value = node})
    let edges = [skillQuantityEdges; skillToDecorationAssignment; decorationToSlotAssignment; slotToSinkEdges] |> List.concat
  ...
  let initializeSearchableMaxFlowPlot (maxFlowPlot:MaxFlowPlot) : SearchableMaxFlowPlot<NodeType> =
    ...
    let edges = 
      [ for edge in maxFlowPlot.Edges -> 
          ref { 
            ...
            Source = nodes |> List.filter (fun node -> node.Value.Vertex.Value = edge.Source) |> List.tryExactlyOne |> Option.get
            Sink = nodes |> List.filter (fun node -> node.Value.Vertex.Value = edge.Sink) |> List.tryExactlyOne |> Option.get
            }
      ]
    ...
    for node in nodes do
      node.Value <- 
        { node.Value with 
            SourceEdges = edges |> List.filter (fun edge -> edge.Value.Sink = node || edge.Value.Source = node)
            SinkEdges = edges |> List.filter (fun edge -> edge.Value.Source = node || edge.Value.Source = node)
        }
  ...
  let validSinkEdge (edge:SearchableEdge<NodeType> ref) = 
    edge.Value.ForwardResidualCapacity > 0

  let validSourceEdge (edge:SearchableEdge<NodeType> ref) = 
    edge.Value.ReverseResidualCapacity > 0

  let breadthFirstSearch (searchableMaxFlowPlot : SearchableMaxFlowPlot<NodeType>) (paths : Path list) : Path list = 
    match paths with
    | [] -> 
      let sources = searchableMaxFlowPlot.Vertices |> List.filter (fun vertex -> vertex.Value.Vertex.Value = Source)
