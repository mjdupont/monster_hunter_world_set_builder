      ...
      testCase "Matches SkillRanks to Skills"
        <| fun _ ->
          ...
          for skill in skills do
            let matchingSkills = allSkillRanksFromDecos |> List.filter (fun sr -> skillRankOfSkill skill sr)
            Expect.isTrue (Set.isSubset (set matchingSkills) (set skill.Ranks)) $"Matching SkillRanks for {skill.Name} must be in the set of SkillRanks in {skill.Name}"
...
let decorationAssignmentFull = 
    ...
    let skillCaps =
      skills |> List.map (fun skill -> skill, (skill.Ranks |> Array.map (fun sr -> sr.Level) |> Array.max))
      |> Map.ofList
    ...
    testList "DecorationAssignmentFull" [ 
      ...
      testCase "Can find a correct mapping from skills to decorations, with smaller decorations"
        <| fun _ ->
          ...
          let slots = [ Slot 4; Slot 4; Slot 4; Slot 4; Slot 2; Slot 1; Slot 1 ] |> List.countBy id
      ...
      testCase "Will assign Hard Decorations when needed"
        <| fun _ ->
          ...
          let availableDecorations =
            List.zip decorations [2;1;1;1;1;1;1]

          let slots = [ Slot 4; Slot 4; Slot 4; Slot 4; Slot 4; Slot 4; Slot 1 ] |> List.countBy id
          ...
          Expect.equal (assignedDecorations |> List.sort) (expectedAssignment |> List.sort) "Failed to correctly assign Hard decorations"
      ...
      testCase "Reserves Slots when requirements are fewer than slots"
        <| fun _ ->
          ...
          Expect.isFasterThan runAssignment threeSeconds "Took longer than three seconds to assign decorations"
          Expect.isSome assignment "Failed to find a decoration assignment"
          Expect.hasCountOf (assignment |> Option.defaultValue []) 2u (fun (slot, deco) -> deco.IsNone) "reserved at least 2 decorations"
