    ...
    testList "MaxFlowMap" [
        ...
        <| fun _ ->
            ...
            let availableDecorations =
                ...
                |> List.groupBy fst
                |> List.map (fun (skill, elements) ->
                    skill,
                    elements
                    |> List.map (fun (skill', (deco, count)) -> (deco, count))
                    |> Map.ofList)
                |> Map.ofList
            ...
            let assignment =
                ...
                |> decorationAssignmentEdges
                |> List.filter (fun edge -> (maxFlowGraph.Flow |> Map.tryFind edge |> Option.defaultValue 0) > 0)
                |> List.map (fun (deco, slot) ->
                    deco
                    |> (function
                    | Decoration d -> Some d
                    | _ -> None)
                    |> Option.get
                    |> (fun deco -> deco.Decoration))
            ...
            Expect.equal
                (assignment |> List.sort)
                (expectedAssignment |> List.sort)
                "Identifies a correct assignment on a larger graph with smaller sized decorations"
    ]